...
class Insight extends Component {
  ...
  toDateTime= ()=> {
    ...
    date = date.replace('T', ' ');
    var t =  date.split('.')[0].split(' ')[0] + ' ' + epoch.toLocaleTimeString().split(' ')[0];
    return t;
  }
      ...
      <View style={{flex: 1}}>
        ...
        <ScrollView style={{padding: 10}}>
          ...
          <FlatList
            ...
            keyExtractor={(x, i) => i.toString()}
            renderItem={({item, index}) => (
              <TouchableOpacity activeOpacity={.9} style={{marginHorizontal: 10, marginBottom: 30, top: 10}}>
                <View style={{
                        flexDirection:'column',
                        backgroundColor: '#fff',
                        borderRadius: 5,
                        ...
                        elevation: 6,
                        padding:10}}>
                  <Text style={{fontWeight:'700' , fontSize:15}}>{item.project_title}</Text>
                  <Text style={{fontSize:13}}>{entities.decode(item.amount)}</Text>
                  <Text style={{fontSize:13 , color:'#767676' }}>{item.timestamp}</Text>
                </View>
              </TouchableOpacity>
            )}
            horizontal={true}
          ...
          <TouchableOpacity
            ...
            <View>
              ...
              <Text style={{color: '#fff', fontSize: 16, fontWeight: '700'}}>
              {entities.decode(
                  this.state.fetchDashboardData.unread_messages
                )} Messages Found
              </Text>
            </View>
